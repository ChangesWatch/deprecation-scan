...
import { catalogSchema, type Catalog, type Dependency, type Finding, type FindingGroup, type ScanReport } from "./types.js";
...
function makeFinding(input: Omit<Finding, "fingerprint" | "package" | "version" | "relationship" | "sourceFile" | "severity" | "source" | "deadlineStatus" | "dependencyPath"> & { dependency: Dependency; identity: string }): Finding {
  ...
  return {
    ...
    source,
    deadlineStatus,
    fingerprint: createHash("sha256").update([identity, dependency.name, dependency.version, dependency.source].join("\0")).digest("hex"),
  };
}

function buildRecommendation(replacementText: string | null, replacementPackage: string | null, strategy: string, replacementVersion: string | null, codemodCommand: string | null): string {
  if (codemodCommand) return `Use the verified codemod: ${codemodCommand}`;
  if (replacementPackage && replacementVersion) return `${strategy === "replace" ? "Replace" : "Upgrade"} to ${replacementPackage}@${replacementVersion}.`;
  if (replacementPackage) return `${strategy === "replace" ? "Replace" : "Upgrade"} with ${replacementPackage}.`;
  if (replacementText) return replacementText;
  return "Review the vendor migration guidance and plan the upgrade.";
}

const severityRank = { urgent: 3, high: 2, attention: 1 } as const;
...
function groupFindings(findings: Finding[]): FindingGroup[] {
  ...
  for (const finding of findings) {
    ...
    if (!current.kinds.includes(finding.kind)) current.kinds.push(finding.kind);
    if (!current.sourceFiles.includes(finding.sourceFile)) current.sourceFiles.push(finding.sourceFile);
    if (!current.dependencyPaths.some((path) => path.join("\0") === finding.dependencyPath.join("\0"))) current.dependencyPaths.push(finding.dependencyPath);
    if (finding.officialSourceUrl && !current.officialSourceUrls.includes(finding.officialSourceUrl)) current.officialSourceUrls.push(finding.officialSourceUrl);
    if (finding.changesWatchUrl && !current.changesWatchUrls.includes(finding.changesWatchUrl)) current.changesWatchUrls.push(finding.changesWatchUrl);
    if (!current.details.includes(finding.detail)) current.details.push(finding.detail);
    if (!current.replacementPackage && finding.replacementPackage) current.replacementPackage = finding.replacementPackage;
    if (!current.migrationUrl && finding.migrationUrl) current.migrationUrl = finding.migrationUrl;
    if (finding.migrationStrategy !== "unknown" && !current.migrationStrategies.includes(finding.migrationStrategy)) current.migrationStrategies.push(finding.migrationStrategy);
    if (finding.codemodCommand && !current.codemodCommands.includes(finding.codemodCommand)) current.codemodCommands.push(finding.codemodCommand);
    if (finding.breakingChanges && !current.breakingChanges.includes(finding.breakingChanges)) current.breakingChanges.push(finding.breakingChanges);
    if (current.recommendation.startsWith("Find a maintained") && !finding.recommendation.startsWith("Find a maintained")) current.recommendation = finding.recommendation;
    if (current.deadlineStatus !== "passed" && finding.deadlineStatus === "passed") current.deadlineStatus = "passed";
    if (!current.effectiveOn || (finding.effectiveOn && finding.effectiveOn < current.effectiveOn)) current.effectiveOn = finding.effectiveOn;
  }
